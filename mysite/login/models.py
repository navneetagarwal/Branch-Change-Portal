...
detail_data = []
roll_row = -1
with open('./login/import_document/input.csv','r') as detail:
...
def validate_CPI(value):
  global roll_row
  if value<0:
    raise ValidationError('%.2f is not a valid CPI' % value)
  if value>10:
    raise ValidationError('%.2f is not a valid CPI' % value)
...
def validate_Roll(value):
    ...
    global null_bool
    null_bool = 0
    if(value/10000000!=15):
      raise ValidationError('%s is not a valid roll number' % value)
    elif(len(str(value))!=9):
      raise ValidationError('%s is not a valid roll number' % value)
    roll = -1
    for x in range(len(detail_data)):
      if value == int(detail_data[x][0]):
        roll = x
    global roll_row
    roll_row = roll
    if roll == -1:
...
def validate_Branch(value):
  global roll_row
  global val_prog
  if value != detail_data[roll_row][2]:
...
def validate_Category(value):
  ...
  global Cat1
  global Cat2
  if value != detail_data[roll_row][4]:
  ...
  if value != 'null':
    if null_bool == 0:
      ...
      if val_prog[index][1] == 0:
        ...
        raise ValidationError('%s is repeated. Please rectify.' % value)
      elif val_prog[index][1] == 2:
        raise ValidationError('%s is your original branch. You can not add your original branch' % value)
    else:
      raise ValidationError("You can't declare preference after putting null.")
  else:
    null_bool = 1

class Detail(models.Model):
    author = models.ForeignKey('auth.User', unique = True, default='',help_text='Enter the proper Roll No provided by the institute.')
    RollNo = models.PositiveIntegerField(primary_key=True, validators=[validate_Roll])
    Name = models.CharField(max_length=80)
    prog_data = []
    with open('./login/import_document/input_programmes.csv', 'r') as csvfile:
...
class DetailAdmin(admin.ModelAdmin):
      ...
      list_display = ('author', 'RollNo', 'Name','Current_Branch','CPI','Category','Preference1')
      ...
      def import_data(self, request, queryset):
        for detail in Detail.objects.all():
          if detail.Name != "new user":
            detail.delete()
        f=open('./login/input_options.csv','r')
